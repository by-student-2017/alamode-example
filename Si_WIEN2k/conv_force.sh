...
for ((i=1; i<=${natom}; i++)); do
  ...
  xu=`awk -v nls=${nls} -v i=${i} '{if(NR>nls && ($1==":POS00"i":" || $1==":POS0"i":" || $1==":POS"i";")){printf "%20.15f",$6}}' ${fname}`
  yu=`awk -v nls=${nls} -v i=${i} '{if(NR>nls && ($1==":POS00"i":" || $1==":POS0"i":" || $1==":POS"i";")){printf "%20.15f",$7}}' ${fname}`
  zu=`awk -v nls=${nls} -v i=${i} '{if(NR>nls && ($1==":POS00"i":" || $1==":POS0"i":" || $1==":POS"i";")){printf "%20.15f",$8}}' ${fname}`
