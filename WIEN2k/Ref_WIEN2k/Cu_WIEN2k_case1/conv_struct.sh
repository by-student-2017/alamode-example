...
echo "MODE OF CALC=RELA unit=bohr" >> ${fname}.struct

xx=`awk '{if($3=="xlo"){printf "%9.6f", $2*1.889726}}' tmp.lammps`
yy=`awk '{if($3=="ylo"){printf "%9.6f", $2*1.889726}}' tmp.lammps`
zz=`awk '{if($3=="zlo"){printf "%9.6f", $2*1.889726}}' tmp.lammps`
...
awk -v nla=${nla} -v elem1=${elem1} -v xx=${xx} -v yy=${yy} -v zz=${zz} '{if(NR>nla && $1>0){
		printf "ATOM %3d: X=%8.6f   Y=%8.6f   Z=%8.6f \n",
			-$1,($3*1.889726/xx),($4*1.889726/yy),($5*1.889726/zz);
